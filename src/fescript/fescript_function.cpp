...
FescriptFunction::FescriptFunction(std::shared_ptr<Function> declaration,
  ...
  this->_is_variadic = this->declaration->is_variadic;
}

[[nodiscard]] int FescriptFunction::arity() {
  return this->_is_variadic ? (this->declaration->params.size() - 1) : this->declaration->params.size();
}
  ...
  for (std::size_t i = 0; i < this->declaration->params.size(); ++i) {
    if(this->declaration->params[i].is_variadic) {
      if(i + 1 >= this->declaration->params.size()) {
        auto variadic_args = std::make_shared<FescriptArray>();
      ...
      fresh::log_error(fresh::src(), "variable that linked with variadic arguments must on the last argument of function");
      return nullptr;
    }
